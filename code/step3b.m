...
for i=1:length(unq)
    ...
    n(i)=length(x); %%% number of agents holding the idea i
    mat1=mat(x,x); %%% focusing on the fraction of the matrices related to agents holding the idea i
    mat2=sparse(mat1); %%% making mat1 sparse in order to passit to shortest_paths function
    sum1=0;
    for j=1:n(i)
        for k=1:n(i)
            dis=graphshortestpath(mat2,j,k); %%% distance of each agent with similar idea from agent j inside the group [needs the bioinformatics toolbox to be installed]
    ...
    if n(i)>1
       ...
       dd(i)=(sum1/2)/norml; %%% average intra_idea distance for idea i
    else
       dd(i)=0;
    end
end
